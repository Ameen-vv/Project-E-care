...
export const getDoctors = (req, res) => {
    try {
        ...
        filter && (filter === 'price-lt-1000' && (query.priceOffline = { $lte: 1000 }))
...
export const bookAppoinment = (req, res) => {
    try {
        ...
        checkingSlotsAvailability({ newDate, doctorId, time })
            .then(({ available, amount }) => {
                if (available) {
                    appointmentModel
                        .findOne({
...
export const verifyPayment = (req, res) => {
    try {
        ...
        if (expectedSignature === req.body.razorpay_signature) {
            appointmentModel
                .updateOne({ _id: req.query.orderId }, {
                    $set: {
                        paymentStatus: true,
            ...
            appointmentModel
                .findOneAndRemove({ _id: req.query.orderId })
                .then(() => {
...
export const getUserDetails = (req, res) => {
    try {
        userModel
            .findOne({ _id: req.userLogged }, { password: 0 })
            .then((user) => {
...
export const editProfile = (req, res) => {
    try {
        const data = req.body
        userModel
            .updateOne({ _id: req.userLogged }, data)
            .then((update) => {
...
export const getWallet = (req, res) => {
    try {
        walletModel
            ...
            .populate('transactions').then((response) => {
                res.status(200).json(response)
            })
    } catch (err) {
        res.status(500)
